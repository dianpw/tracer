                    ...
                    <div class="col-md-9">
                        <div class="m-alert m-alert--icon m-alert--air alert alert-dismissible fade show" role="alert" style="background-color: #95a57e36">
                            ...
                            <div class="m-alert__text">
                                ...
                                <b>F6.</b> Berapa perusahaan/instansi/institusi yang sudah anda lamar (lewat surat atau e-mail) sebelum anda memeroleh pekerjaan pertama?<br>
                                <b>F7.</b> Berapa banyak perusahaan/instansi/institusi yang merespons lamaran anda? <br>
                                <b>F7a.</b> Berapa banyak perusahaan/instansi/institusi yang mengundang anda untuk wawancara?
                            </div>
                           
                        </div>
                    </div>
                    <div class="col-md-3 text-right">
                        <a href="{{url('export_kemulusan_transisi')}}" class="btn btn-success m-btn m-btn--custom m-btn--icon">
                            <span>
                ...
                <table cellpadding="0" class="table-striped tabel_show table table-bordered nowrap" cellspacing="0" width="100%">
                    <thead style="background-image: url('https://www.transparenttextures.com/patterns/sos.png');background-color: #349d44;color: #e5f6dd;">
                        <tr>
                            ...
                            <th style="15%">Perusahaan Dilamar</th>
                            <th style="25%">Perusahaan Merespon</th>
                            <th style="25%">Perusahaan Mengundang wawancara</th>
...
<script>
    $(document).ready(function(){
        var table_evaluasi_pegawai = $('.tabel_show').DataTable({
            ...
            "ordering": true,
            ajax: {
                url: '{{url("datatable-kemulusan-transisi")}}',
                dataSrc: 'result',
            },
            scrollX:        true,
            autoWidth:         true,
    ...
    $('#change_graph').select2({width: 'resolve'});
        ...
        function chart(tahun){
        	$.ajax({
                url: "{{url('graph-kemulusan-transisi')}}" + '/' + tahun,
                type: 'GET',
                dataType: 'JSON',
                success:function(response){

                    Highcharts.chart('kemulusan_transisi', {
                        chart: {
                            type: 'column'
                        },
                        title: {
                            text: 'Kemulusan Transisi - Tahun lulus: '+'<b style="color:#1978a9">' +tahun+'</b><br><br>'
                        },
                        subtitle: {
                            text: 'Source: <a href="">http://cdc.unisma.ac.id/tracer_study/</a>'
                        },
                        xAxis: {
                            categories: ['Perusahaan dilamar',
                                        'Perusahaan Merespon',
                                        'Perusahaan mengundang wawancara',
                                        ],
                            title: {
                                text: null
                            }
                        ...
                        series: [{
                            data: [
                                {name: 'Perusahaan yang dilamar', y:response.perusahaan_dilamar[0], color:'#0074D9'},
                                {y:response.perusahaan_merespon[0], color:'red'},
                                {y:response.perusahaan_mengundang_wawancara[0], color:'green'},
                            ],
