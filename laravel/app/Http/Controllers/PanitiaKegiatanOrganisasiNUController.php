...
class PanitiaKegiatanOrganisasiNUController extends Controller
{
    public function index()
    {
        ...
        return view('panitia_kegiatan_organisasi_nu.index', compact('auth_user', 'show_tahun','tahun_terakhir'));
    }
    public function graph($tahun=null){
        $prodi_id = '';
        if(Auth::user()->role_id == 1){
           $prodi_id = 'where pengaruh_organisasi is not null and tb_mahasiswa.npm is not null and YEAR(tahun_lulus) = "'.$tahun.'"';
        }else if(Auth::user()->role_id == 2){
            $prodi_id = ' where mhs_fakultas_id= "'.Auth::user()->fakultas_user_id.'" and pengaruh_organisasi is not null and tb_mahasiswa.npm is not null and YEAR(tahun_lulus) = "'.$tahun.'"';
        }else if(Auth::user()->role_id == 3){
            $prodi_id = ' where kode_prodi_id = "'.Auth::user()->prodi_id.'" and pengaruh_organisasi is not null and tb_mahasiswa.npm is not null and YEAR(tahun_lulus) = "'.$tahun.'"';
        }
        $query = DB::select('SELECT pengaruh_organisasi, COUNT(*) as jumlah  FROM `tb_pengaruh_organisasi`
        LEFT JOIN tb_mahasiswa ON tb_pengaruh_organisasi.npm = tb_mahasiswa.npm
        '.$prodi_id.' GROUP BY pengaruh_organisasi'
        );


        $data['pengaruh_organisasi'] =[];

        foreach ($query as $key => $value) {
            array_push($data['pengaruh_organisasi'],
            [ $value->pengaruh_organisasi,
              $value->jumlah,
             false,
            ]);
    ...
    public function show_data(){
        try {
            ...
            if(Auth::user()->role_id == 1){
                $query = DB::table('tb_pengaruh_organisasi')
                ->Leftjoin('tb_mahasiswa', 'tb_pengaruh_organisasi.npm',  'tb_mahasiswa.npm')
    ...
    public function exportExcel(){
        ...
        $sheet->setCellValue('B2', 'LAPORAN TRACER STUDY - Pengaruh pengaruh_organisasi dengan Capaian Karir');
        ...
        $sheet->setCellValue('F3', 'Pengaruh pengaruh_organisasi');
        ...
        foreach ($query as $key => $value) {
            ...
            $sheet->setCellValue('F'.$i, $value->pengaruh_organisasi);
        ...
        $date = date('d-F-Y');
        $file_name = 'Laporan Pengaruh pengaruh_organisasi dengan Capaian Karir - '. $date.'.xlsx';
