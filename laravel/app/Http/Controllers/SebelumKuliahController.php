...
class SebelumKuliahController extends Controller
{
    public function index()
    {
        ...
        return view('sebelum_kuliah.index', compact('auth_user', 'show_tahun', 'tahun_terakhir'));
    }
    ...
    public function graph($tahun=null){
        ...
        $query = DB::select('SELECT riwayat_kerja,
        COUNT(*) AS jumlah  FROM tb_riwayat_s1
        JOIN tb_mahasiswa ON tb_riwayat_s1.npm = tb_mahasiswa.npm'.$prodi_id. 'group by riwayat_kerja');

        $data['riwayat'] =[];

        foreach ($query as $key => $value) {
            array_push($data['riwayat'],
            [ $value->riwayat_kerja,
              $value->jumlah,
              false,
            ]);
    ...
    public function show_data(){
        try {
            ...
            if(Auth::user()->role_id == 1){
                $query = DB::table('tb_riwayat_s1')
                ->Leftjoin('tb_mahasiswa', 'tb_riwayat_s1.npm',  'tb_mahasiswa.npm')
    ...
    public function exportExcel(){
        ...
        $sheet->setCellValue('B2', 'LAPORAN TRACER STUDY - Karakteristik Sosio-Biografi');
        ...
        $sheet->setCellValue('F3', 'UNIVERSITAS S1');
        $sheet->setCellValue('G3', 'PRODI S1');
        $sheet->setCellValue('H3', 'TAHUN MASUK S1');
        $sheet->setCellValue('I3', 'RIWAYAT KERJA SEBELUM S2');
        ...
        foreach ($query as $key => $value) {
            ...
            $sheet->setCellValue('F'.$i, $value->univs1);
            $sheet->setCellValue('G'.$i, $value->prodis1);
            $sheet->setCellValue('H'.$i, $value->masuks1);
            $sheet->setCellValue('I'.$i, $value->riwayat_kerja);
            $i++;
        }

        $writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, "Xlsx");
        $date = date('d-F-Y');
        $file_name = 'Laporan Karakteristik Sosio-Biografi - '. $date.'.xlsx';
