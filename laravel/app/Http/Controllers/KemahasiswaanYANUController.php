...
class KemahasiswaanYANUController extends Controller
{
    ...
    $tahun_terakhir = DB::select('SELECT MAX(tahun_lulus) AS tahun_lulus FROM tb_mahasiswa');
        return view('kemahasiswaan_ya_nu.index', compact('auth_user','show_tahun', 'tahun_terakhir'));
    }
    public function graph($tahun=null){
        $prodi_id = '';
        if(Auth::user()->role_id == 1){
           $prodi_id = 'where ya_nu is not null and tb_mahasiswa.npm is not null and tahun_lulus = "'.$tahun.'"';
        }else if(Auth::user()->role_id == 2){
            $prodi_id = ' where mhs_fakultas_id= "'.Auth::user()->fakultas_user_id.'" and ya_nu is not null and tb_mahasiswa.npm is not null and tahun_lulus = "'.$tahun.'"';
        }else if(Auth::user()->role_id == 3){
            $prodi_id = ' where kode_prodi_id = "'.Auth::user()->prodi_id.'" and ya_nu is not null and tb_mahasiswa.npm is not null and tahun_lulus = "'.$tahun.'"';
        }

        $query = DB::select('SELECT ya_nu, COUNT(*) as jumlah  FROM `tb_ya_organisasi_nu`
        LEFT JOIN tb_mahasiswa ON tb_ya_organisasi_nu.npm = tb_mahasiswa.npm
        '.$prodi_id.' GROUP BY ya_nu'
        );
    ...
    public function show_data(){
        try {
            ...
            if(Auth::user()->role_id == 1){
                $query = DB::table('tb_ya_organisasi_nu')
                ->Leftjoin('tb_mahasiswa', 'tb_ya_organisasi_nu.npm',  'tb_mahasiswa.npm')
    ...
    public function exportExcel(){
        ...
        $sheet->setCellValue('B2', 'LAPORAN TRACER STUDY - Kemahasiswaan YA NU');
        ...
        $sheet->setCellValue('F3', 'Kemahasiswaan YA NU');
        ...
        foreach ($query as $key => $value) {
            ...
            $sheet->setCellValue('F'.$i, $value->ya_nu);
        ...
        $date = date('d-F-Y');
        $file_name = 'Laporan Kemahasiswaan YA NU - '. $date.'.xlsx';
