...
class MasaTungguController extends Controller
{
    public function index()
    {
        ...
        return view('masa_tunggu.index', compact('auth_user', 'show_tahun', 'tahun_terakhir'));
    }
    public function graph($tahun=null){
        ...
        $query_sebelum_kelulusan = DB::select('SELECT  bulan, masa_tunggu AS wmmp,
        COUNT(masa_tunggu) AS jumlah  FROM `tb_masa_tunggu`
        JOIN tb_mahasiswa ON tb_masa_tunggu.npm = tb_mahasiswa.npm
        where masa_tunggu = "sebelum" '.$prodi_id. 'group by bulan');
        $query_sesudah_kelulusan = DB::select('SELECT  bulan, masa_tunggu AS wmmp,
        COUNT(masa_tunggu) AS jumlah  FROM `tb_masa_tunggu`
        JOIN tb_mahasiswa ON tb_masa_tunggu.npm = tb_mahasiswa.npm
        where masa_tunggu = "setelah" '.$prodi_id. 'group by bulan');

    //   dd($query_sesudah_kelulusan);


        $data = [];
        foreach ($query_sebelum_kelulusan as $key => $value) {
            array_push($data, [$value->bulan. ' Bulan sebelum lulus ujian', (int)$value->jumlah]);

        }
        foreach ($query_sesudah_kelulusan as $key => $value) {
            array_push($data, [$value->bulan. ' Bulan sesudah lulus ujian',  (int)$value->jumlah]);
    ...
    public function show_data(){
        try {
            ...
            if(Auth::user()->role_id == 1){
                $query = DB::table('tb_masa_tunggu')
                ->Leftjoin('tb_mahasiswa', 'tb_masa_tunggu.npm',  'tb_mahasiswa.npm')
    ...
    public function exportExcel(){
        ...
        $sheet->setCellValue('B2', 'LAPORAN TRACER STUDY - Masa Tunggu');
        ...
        $sheet->setCellValue('F3', 'Masa Tunggu');
        ...
        foreach ($query as $key => $value) {
            ...
            $sheet->setCellValue('F'.$i, $value->masa_tunggu);
        ...
        $date = date('d-F-Y');
        $file_name = 'Laporan Masa Tunggu - '. $date.'.xlsx';
